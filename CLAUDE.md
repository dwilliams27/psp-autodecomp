...
- **Compiler flag layout**: Most code uses `-Xsched=2` (SNC default). `eAll_psp.obj` is a **unity build** (`eAll_psp.cpp`) that uses `#pragma` to switch sched mid-file. The old address-only rule for the transition zone was too coarse: `0x040000-0x08f000` is mixed at class/method granularity. The Makefile defaults to `sched=2` with per-class pattern overrides for confirmed `sched=1` exceptions, but transition-zone work should usually use local `#pragma control sched=N` around individual methods. See `docs/research/snc-transition-zone-sched.md`.
...
- **Clean overnight runs may be merged proactively.** When a run finishes and every outcome is clean — matches verified byte-exact, attribution complete, no system errors, no ambiguous overload classifications, no quality-gate rejections needing human review, no Mode-B/shootout branches that require `tools/ab_promote.py` — `git merge --ff-only overnight/<ts>` into main plus branch delete is authorized without asking. Show `git log --oneline -5` after the merge so the operator sees what landed. Pause and ask if anything needs triage: mixed clean/unclean outcomes, circuit-breaker stops, Bug-#2 contamination, or any cherry-pick scenario. Non-fast-forward merges always require a confirmation since they imply main moved during the run.
