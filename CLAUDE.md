...
# write/edit C in src/
python3 tools/compare_func.py src/file.cpp --no-update-db        # compile + compare (read-only default)
...
**Agent guardrails:**
- Agents must not modify `tools/`, `config/`, or `Makefile`. The orchestrator owns `config/functions.json` status/provenance updates; use `compare_func.py --update-db` only during manual maintenance. Report tool bugs as session errors.
