#!/usr/bin/env python3
"""Portal helix — ends anchored (pinched to zero radius at both portals).

Same double helix as v7, but the radius envelope is now r(u) = R * sin(π·u),
so the coil pinches to a point at both portal centers. Characters appear to
emerge from the small portal as a single-file stream, fan out to full spiral
width in the middle, then contract back to a point entering the large portal.

Run: python3 tools/launcher_demos/v8_portal_helix_fixed.py
"""
...
TURNS = 2.2
R_PEAK = 4.0          # maximum radius (at u=0.5)
Y_ASPECT = 0.55
...
def _depth_style(z, r):
    # Normalize by the current peak radius (not a fixed constant), otherwise
    # pinched ends show only the middle of the ramp.
    if r < 0.01:
...
def _render_frame(t):
    ...
    for strand_ix in range(2):
        ...
        for j in range(N_SLOTS):
            ...
            # Sinusoidal radius envelope: 0 at u=0, peak at u=0.5, 0 at u=1
        ...
        try:
            while True:
                ...
                panel = Panel(
                    ...
                    title=Text(" decomp stream — fixed ends ", style="bold color(178)"),
