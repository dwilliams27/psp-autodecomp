...
def build_prompt(batch, functions, session_id):
    ...
    prompt_parts = [
        ...
        "free_functions.c or in a different class's .cpp file. "
        "Do NOT use extern-C safe-name wrappers for "
        ...
        "   - Known SNC register-allocation drift? If the diff is <=8 bytes "
        "and only renames callee-save registers in sw/lw prologue/epilogue, "
        "move sN,sM, or arithmetic register fields, read "
        "docs/research/snc-register-allocation.md, run at most one permuter, "
        "then stop and report failed with category=REG_ALLOC. Source usually "
        "has no leverage over this pattern.\n"
