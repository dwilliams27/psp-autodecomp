"""tier12b prompt variant. Slimmed-down tier12 after the 2026-04-21 A/B run.

The first A/B (base vs tier12) was a null result on match rate
(77.6% vs 77.9%, χ²=0.002, p>>0.10). tier12 successfully changed agent
behavior for INFORMATIONAL directives — research-doc reads went 6× up,
sibling-pattern citations 2.4× up, permuter mentions 60% up — but
PROHIBITION-style directives were ignored (attempts:0 spiked 0→10;
system-reminder abort still fired).

Changes vs tier12:
  - DROP: 'system reminders are not stop signals' disclaimer (ignored)
  - DROP: 'never report attempts:0 for batch siblings' (backfired)
  - KEEP: research library, pattern-copy-first step 0, expanded
          mismatch-diagnosis recipes, last-mile tools (permuter + pragma),
          unmatchable_bnel 5-criteria, JSON example, matched-neighbors
          'read them first' hint, prior-notes guidance.

See docs/postmortems/2026-04-21-overnight-ab1.md.
"""
...
def build_prompt(batch, functions, session_id, class_to_header=None):
    ...
    prompt_parts = [
        ...
        "- CLASS METHOD FILE RULE: Every class method MUST use canonical "
        "C++ syntax: "
        "`ReturnType ClassName::MethodName(args) { ... }`. "
        "DESTRUCTORS: use `ClassName::~ClassName() { ... }` — "
        "never `extern \"C\" void __0o...` wrappers. "
        "Default to src/<ClassName>.cpp. Do NOT put class methods in "
        "free_functions.c or another class's file. "
        "Do NOT use extern-C safe-name wrappers — the quality gate rejects them.\n"
        "- SPLIT-TU METHOD-ADDITION RULE: If your match requires new methods "
        "on an existing class, do NOT edit include/*.h. Create "
        "src/<Class>_<Method>.cpp with a local class redeclaration containing "
        "only the methods this TU references. Add an ODR-WARNING comment at "
        "the top. See src/eDynamicModel_SetGeomFlagsOnOff.cpp for the pattern "
        "and docs/direction/005-regalloc-drift-guards.md for why header edits "
        "can regress matched siblings.\n"
        ...
        "  <=8B s0/s1/s2 register swap     →  docs/research/snc-register-allocation.md\n\n"
        ...
        "   KNOWN SNC REGISTER-ALLOCATION DRIFT:\n"
        "     If the diff is <=8 bytes and only renames callee-save registers "
        "in sw/lw prologue/epilogue, move sN,sM, or arithmetic register fields, "
        "read docs/research/snc-register-allocation.md, run at most one permuter, "
        "then stop. Report failed with category=REG_ALLOC in notes. Source "
        "usually has no leverage over this pattern.\n\n"
        ...
        "LAST-MILE TOOLS (use only after compare_func shows the generated "
        "symbol is the same size as the DB target and diff ≤30 bytes):\n\n"
        "  tools/permuter.py — automated source mutation + compile + compare.\n"
        "    For diffs ≤16B:  python3 tools/permuter.py src/<file>.cpp 0x<addr> "
        "--time 600 --workers 2 --save-best\n"
        "    For diffs 17-30B: python3 tools/permuter.py src/<file>.cpp 0x<addr> "
        "--time 300 --workers 2\n"
        "    To keep a non-exact improvement: add BOTH --save-best "
        "--save-improved. --save-improved alone is invalid.\n"
        "    If the permuter prints PERMUTER_SKIPPED_NOT_LAST_MILE, fix source "
        "shape/size first instead of forcing a long run.\n"
        ...
        "  Flag sweep (advanced): use permuter --fprreserve N / --gprreserve N "
        "and VFPU --vfpumatrix N / --vfpuscalar N when the source needs those "
        "compiler flags. Do NOT edit the Makefile.\n\n"
        ...
        f"that retries this function — make them useful. Keep the 'file' field set "
        f"to the source file containing your closest attempt, even when status is "
        f"'failed'; the orchestrator snapshots that source for retries.\n\n"
