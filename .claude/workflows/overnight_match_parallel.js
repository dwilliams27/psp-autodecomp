...
function buildPrompt(t) {
  ...
  return [
    ...
    '6b. EXACT-SIZE REG_ALLOC residual (right instructions, wrong registers/callee-save coloring — the ADR-012 Mechanism-B class)? It is OFTEN SOURCE-FIXABLE. Toolkit, in order: (i) INDUCTION-VARIABLE idiom on loops — array index a[i] vs pointer-walk (p; p++), base-pointer kept-live vs a mutated loop pointer; this flips the allocator live-range order (it cracked cFactory::WriteGroups 9B->0 and others). (ii) local DECLARATION ORDER. (iii) variable NAMING/name-hash. (iv) introduce/inline a temp, split a live range, sink/hoist. (v) commutative-operand and inline-asm operand-list order. (vi) CLOSER: python3 tools/permuter.py src/' + t.safe_name + '.cpp ' + t.address + ' --time 600 --save-best --score-mode insns  (for VFPU funcs ALSO pass --vfpumatrix 1 --vfpuscalar 8). BANNED even if it reaches byte-exact: register T __asm__("$reg") pinning, hand-written jr/b, hardcoded $aN arg reads, __attribute__((naked)) whole-body asm, integer-MIPS asm that materializes a C-expressible value (e.g. ori %0,$0,1 to force a constant), AND synthetic `volatile` dead-store/dummy-local crutches inserted only to emit a phantom store that fakes a matching byte (a meaningless `volatile int scratch=0;` with no program meaning is gaming) — those are codegen-gaming, not matches. If exact-size with only a small register/scheduler coloring residual remains after honest source levers + permuter, return near_miss; do NOT reach for a gaming crutch.',
    '',
