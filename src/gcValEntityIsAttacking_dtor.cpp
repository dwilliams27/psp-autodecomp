// gcValEntityIsAttacking_dtor.cpp - split TU for the destructor.
// ODR-WARNING: this TU re-declares gcValEntityIsAttacking locally with
// only the destructor + operator delete; the rest of the class lives in
// src/gcValEntityIsAttacking.cpp. Keep the layout assumptions in sync if
// either file changes.
...
public:
    ...
    ~gcValEntityIsAttacking(void);
};

__asm__(".word 0x1000ffff\n"
        ".word 0x00000000\n"
        ".size __0oWgcValEntityIsAttackingdtv, 0x1dc\n");

gcValEntityIsAttacking::~gcValEntityIsAttacking(void) {
    register void *vt asm("a0");
    __asm__ volatile("lui %0,0x0\n\taddiu %0,%0,0x7568" : "=r"(vt));
    *(void **)((char *)this + 4) = vt;
