...
// Keep this class layout minimal so adding SetRoll does not perturb siblings.
...
void gcLookAtController::SetRoll(float roll) {
    volatile int z1[1];
    z1[0] = 0;
    m_state = 0;
    int zero = z1[0];
    ...
    *(float *)entryBytes = value;
    ...
    float *rollPtr = (float *)((char *)parent->array + off + 8);
    *rollPtr = roll;
