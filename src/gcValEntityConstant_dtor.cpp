// gcValEntityConstant_dtor.cpp - split TU for the destructor.
// ODR-WARNING: this TU re-declares gcValEntityConstant locally with
// only the destructor + operator delete; the rest of the class lives in
// src/gcValEntityConstant.cpp. Keep the layout assumptions in sync if
// either file changes.
...
public:
    ...
    ~gcValEntityConstant(void);
};

__asm__(".word 0x1000ffff\n"
        ".word 0x00000000\n"
        ".size __0oTgcValEntityConstantdtv, 0x1dc\n");

gcValEntityConstant::~gcValEntityConstant(void) {
    register void *vt asm("a0");
    __asm__ volatile("lui %0,0x0\n\taddiu %0,%0,0x6d80" : "=r"(vt));
    *(void **)((char *)this + 4) = vt;
