// ── dcast<gcStateFunction *>(const cBase *) @ 0x00222b10 ──
// RTTI downcast (gcAll_psp.obj): lazily register gcStateFunction's cType via a
...
// 3-level shape; constants read from THIS function's 0x00222b10 disassembly:
...
//   outer  D_0009A3CC : InitializeType(0,0,0xC8, D_000385E0, &gcStateFunction::New, 0,0,0)
// cNamed::New @ 0x1C3C58.  gcStateFunction::New @ 0x256830.
...
extern "C" {
    ...
    cBase *gcStateFunction__New_cMemPoolptr_cBaseptr(cMemPool *, cBase *);
}
...
T dcast(const cBase *base) {
    ...
    if (base != 0) {
        if (D_0009A3CC == 0) {
            ...
            D_0009A3CC = cType::InitializeType(
                0, 0, 0xC8, D_000385E0,
                (cBase * (*)(cMemPool *, cBase *))&gcStateFunction__New_cMemPoolptr_cBaseptr,
                0, 0, 0);
        }

        const void *classDesc = *(const void **)((const char *)base + 4);
        cType *target = D_0009A3CC;
...
template gcStateFunction *dcast<gcStateFunction *>(const cBase *base);
