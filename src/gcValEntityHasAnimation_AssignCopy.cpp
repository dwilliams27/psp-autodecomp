// gcValEntityHasAnimation_AssignCopy.cpp - split TU for AssignCopy.
...
// src/gcValEntityHasAnimation.cpp. The type cache statics use asm() linkage
// to point to the same memory used by gcValEntityHasAnimation::GetType.
...
__asm__(".word 0x1000ffff\n"
        ".word 0x00000000\n"
        ".size __0fXgcValEntityHasAnimationKAssignCopyPC6FcBase, 0x2d8\n");

void gcValEntityHasAnimation::AssignCopy(const cBase *base) {
    const gcValEntityHasAnimation *other = 0;
    ...
    if (base != 0) {
        if (!type_gcValEntityHasAnimation) {
            ...
            type_gcValEntityHasAnimation = cType::InitializeType(
                0, 0, 0x111, type_value,
                gcValEntityHasAnimation::New, 0, 0, 0);
        }

        void *classDesc = *(void **)((const char *)base + 4);
        cType *wanted = type_gcValEntityHasAnimation;
...
fail:
        ...
        if (ok != 0) {
            other = (const gcValEntityHasAnimation *)base;
        }
    }
