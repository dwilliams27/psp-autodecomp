// eDynamicMeshNode::Read(cReadBlock &) — 0x0004cd10
//
// ODR-WARNING: split-TU local redeclarations. The class layout and helper
// signatures are declared locally so this flat method view does not clash with
// the inheritance/forward views used by other TUs. No new declarations are
// added to include/*.h.
//
// Structure mirrors the matched sibling eMeshShapeNode::Read (0x4ea6c):
// the incoming cReadBlock& holds the cFileHandle* in its first slot
// (lw a0,0(s0); lw a0,0(a0)). Four cFileSystem::Read calls copy bytes into
// this+0 (4), this+0x40 (12), this+0x10 (12), this+0x20 (12), this+0x30 (12),
// then cName::Read(this+0x50, rb&). Arg order is (h=handle, dest, size).
//
// sched=1 prologue/epilogue (eAll transition zone): save s0/s1/ra, s0=rb&,
// s1=this, the reads, restore, jr.

class cFileHandle;
class cReadBlock;

extern "C" {
    void cFileSystem__Read_cFileHandleptr_voidptr_unsignedintstatic(
        cFileHandle *, void *, unsigned int);
    void cName__Read_cReadBlockref(void *, cReadBlock &);
}

class eDynamicMeshNode {
public:
    void Read(cReadBlock &rb);
};

#pragma control sched=1

void eDynamicMeshNode::Read(cReadBlock &rb) {
    void *rbp = (void *)&rb;
    void *self = (void *)this;
    __asm__ volatile("" : "+r"(rbp), "+r"(self) :: "memory");

    cFileHandle *h = *(cFileHandle **)*(void **)rbp;
    cFileSystem__Read_cFileHandleptr_voidptr_unsignedintstatic(
        h, (char *)self + 0, 4);

    h = *(cFileHandle **)*(void **)rbp;
    cFileSystem__Read_cFileHandleptr_voidptr_unsignedintstatic(
        h, (char *)self + 0x40, 0xC);

    h = *(cFileHandle **)*(void **)rbp;
    cFileSystem__Read_cFileHandleptr_voidptr_unsignedintstatic(
        h, (char *)self + 0x10, 0xC);

    h = *(cFileHandle **)*(void **)rbp;
    cFileSystem__Read_cFileHandleptr_voidptr_unsignedintstatic(
        h, (char *)self + 0x20, 0xC);

    h = *(cFileHandle **)*(void **)rbp;
    cFileSystem__Read_cFileHandleptr_voidptr_unsignedintstatic(
        h, (char *)self + 0x30, 0xC);

    cName__Read_cReadBlockref((char *)self + 0x50, *(cReadBlock *)rbp);
}

#pragma control sched=2
