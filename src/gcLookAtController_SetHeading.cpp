...
// Keep this class layout minimal so adding SetHeading does not perturb siblings.
...
void gcLookAtController::SetHeading(float heading) {
    volatile int z1[1];
    z1[0] = 0;
    m_state = 0;
    int zero = z1[0];
    ...
    *target = zero;
    ...
    float *headingPtr = (float *)((char *)array + off);
    *headingPtr = heading;

    LookAtEntry *entry = (LookAtEntry *)((char *)parent->array + off);
    float value;
    int newEntry = isNewLookAtEntry(entry->flags);
    if (newEntry) {
        value = 0.0f;
    } else {
        newEntry = (int)((char *)entry + 4);
    ...
    *(float *)entryBytes = value;
    ...
    newEntry = isNewLookAtEntry(entry->flags);
    if (newEntry) {
        value = 0.0f;
    } else {
