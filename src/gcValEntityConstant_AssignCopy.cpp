// ODR-WARNING: split-TU local redeclaration for one gcValEntityConstant method.
...
extern cType *type_gcValEntityConstant asm("D_0009F7F8");

void gcValEntityConstant::AssignCopy(const cBase *base) {
    const gcValEntityConstant *other = 0;
    char *destValue = (char *)this + 0x48;

    if (base != 0) {
        if (!type_gcValEntityConstant) {
            if (!type_value) {
                if (!type_expression) {
                    if (!type_base) {
            ...
            type_gcValEntityConstant = cType::InitializeType(
                0, 0, 0xD4, type_value, gcValEntityConstant::New,
                0, 0, 0);
        }

        void *classDesc = *(void **)((const char *)base + 4);
        cType *wanted = type_gcValEntityConstant;
        ...
        if (ok != 0) {
            other = (const gcValEntityConstant *)base;
        }
    }
    ...
    *(signed char *)((char *)this + 0x3C) =
        *(const signed char *)((const char *)other + 0x3C);
    *(signed char *)((char *)this + 0x3D) =
        *(const signed char *)((const char *)other + 0x3D);
    {
        const int *srcWord40 = (const int *)((const char *)other + 0x40);
        int *dstWord40 = (int *)((char *)this + 0x40);
        *dstWord40 = *srcWord40;
        const int *srcWord44 = (const int *)((const char *)other + 0x44);
        int *dstWord44 = (int *)((char *)this + 0x44);
        *dstWord44 = *srcWord44;
    }

    if ((const char *)other + 0x48 != destValue) {
        int oldValue = *(int *)((char *)this + 0x48);
        int keep = 1;
        int tag = oldValue & 1;
        if (tag != 0) {
            keep = 0;
        ...
        int srcValue = *(int *)((const char *)other + 0x48);
        int cloneIt = 1;
        ...
        if (cloneIt != 0) {
            ...
            int current = *(int *)((char *)this + 0x48);
            int currentTagged = 0;
            cBase *cloneArg;
            if (current & 1) {
                currentTagged = 1;
