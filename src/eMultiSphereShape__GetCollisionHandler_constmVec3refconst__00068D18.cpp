// eMultiSphereShape::GetCollisionHandler(const mVec3 &) const
// 0x00068d18, 228B, eAll_psp.obj
// Split-TU: eMultiSphereShape declared locally (transition-zone helper omitted
// from the shared header). ODR-WARNING: local class shape; do not include the
// real header here.
//
// Builds the lazily-initialised embedded multi-sphere collision handler, fills a
// stack mCapsule (two sphere centres displaced +/- mHalfLength along the local
// "up" axis C000, scaled radius) from the query position, then dispatches to
// eEmbedMultiSphereCollisionHandler::Set(const mCapsule &). The VFPU sequence has
// no scalar C equivalent (matrix column ops); the FPU/integer glue is expressed
// in C so the SNC scheduler interleaves it as in the original.
...
extern int eMultiSphereShape_vec_handler_guard asm("D_00018E38");
extern struct eMultiSphereShape_VecHandlerStorage
    eMultiSphereShape_vec_handler_storage asm("D_00018E48");

struct eMultiSphereShape_VecHandlerStorage {
    char pad00[0x30];
    int field30;        // 0x30
    char pad34[0x8];
    void *vtable;       // 0x3C
...
int eMultiSphereShape::GetCollisionHandler(const mVec3 &pos) const {
    eMultiSphereShape_VecHandlerStorage *handler =
        &eMultiSphereShape_vec_handler_storage;

    if (eMultiSphereShape_vec_handler_guard == 0) {
        eMultiSphereShape_vec_handler_guard = 1;
        handler->field30 = 0;
