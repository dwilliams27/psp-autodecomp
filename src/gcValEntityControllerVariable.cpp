...
// 0x00327138 (260B) — New (static factory)
cBase *gcValEntityControllerVariable::New(cMemPool *pool, cBase *parent) {
    void *block = ((void **)pool)[9];
    char *allocTable = ((PoolBlock *)block)->allocTable;
    AllocEntry *entry = (AllocEntry *)(allocTable + 0x28);
    short off = entry->offset;
    void *base = (char *)block + off;
    gcValEntityControllerVariable *result = 0;
    gcValEntityControllerVariable *obj =
        (gcValEntityControllerVariable *)entry->fn(base, 0x38, 4, 0, 0);
    if (obj != 0) {
        ((char **)obj)[1] = cBaseclassdesc;
        ((int *)obj)[0] = (int)parent;
        ((char **)obj)[1] = gcValEntityControllerVariablevirtualtable;
        char *sub = (char *)obj + 8;
...
static cType *type_gcValEntityControllerVariable;

const cType *gcValEntityControllerVariable::GetType(void) const {
    if (!type_gcValEntityControllerVariable) {
        if (!type_variable) {
            if (!type_value) {
                if (!type_expression) {
                    if (!type_base) {
        ...
        type_gcValEntityControllerVariable = cType::InitializeType(
            0, 0, 0xCB, type_variable, gcValEntityControllerVariable::New,
            0, 0, 0);
    }
    return type_gcValEntityControllerVariable;
}
