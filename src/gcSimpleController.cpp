...
// ── gcSimpleController::Read(cFile &, cMemPool *) @ 0x00157FB0 ──
int gcSimpleController::Read(cFile &file, cMemPool *pool) {
    int result;
    cReadBlock rb(file, 1, true);
    __asm__("ori %0, $0, 1" : "=r"(result));
    if ((unsigned int)rb._data[3] == 1 && gcEntityController::Read(file, pool)) goto success;
