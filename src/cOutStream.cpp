...
void cOutStream::Write(unsigned int value, int numBits, bool sign) {
    if (sign) {
        if (numBits >= 3) {
            unsigned int isShort = (value == 0) | (value == 1);
            ...
            register unsigned int bitValue __asm__("$t1") = isShort & 0xFF;
            mBitPos = newBitPos;
            if (overflow != 0) {
                overflow = mOverflow;
            } else {
            ...
            if (isShort != 0) {
                ...
                unsigned int bitValue2 = (value2 == 1);
                bitValue2 &= 0xFF;
                int bitPos2 = mBitPos;
