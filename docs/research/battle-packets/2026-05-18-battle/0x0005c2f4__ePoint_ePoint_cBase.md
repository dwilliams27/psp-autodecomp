...
- score: `174`
- priority: `1`
- reason: `has failure_notes; rich notes; has failure snapshot; near miss 6B; codegen-specific diagnosis; Constructor retry family; manageable size`
...
- session `c98dfc39`; src `src/ePoint.cpp`; snapshot `logs/failure_snapshots/match_20260517_210450/c98dfc39/0005c2f4__agent_self_reported_failure__src_ePoint.cpp`

Implemented ePoint::ePoint(cBase *) as a canonical C++ constructor in src/ePoint.cpp, with cObject(parent), ePoint vtable store, D_00038578/D_000385B8 identity initialization, and the color field store. Tried native mOCS/SceULong128 copies, whole-struct copy with memory barriers, volatile row copies, sched=1, and inline VFPU init/copy variants. The closest exact-size source is 15/304 bytes off: the VFPU identity body is structurally aligned, but SNC still emits a 16-byte prologue/epilogue for the inline-asm-heavy shape instead of the original 192-byte frame with s0/ra at 0xB0/0xB4. Variants that force the 192-byte frame through real C row stores either shrink to 300B or produce 50+ byte VFPU register/scheduling diffs because the compiler changes the row-copy order and C120/C130 allocation.

Ran tools/permuter.py for 600s with 2 workers on the exact-size 15-byte baseline: 5308 candidates generated, 2235 compiled, zero improvements, best remained 15 bytes. Root cause remains the same frame/layout lever as the prior attempt: source can force the large frame only by introducing real C stack row operations, and those perturb the desired VFPU copy sequence; pure inline asm gives the desired body but not the compiler-generated frame.
