...
- session `d5b36e6a`; src `src/gcUIWidget_Reset.cpp`; snapshot `logs/failure_snapshots/match_20260517_210450/d5b36e6a/000d8654__agent_self_reported_failure__src_gcUIWidget_Reset.cpp`

Created/retained split-TU canonical C++ source for gcUIWidget::Reset(cMemPool *, bool) in src/gcUIWidget_Reset.cpp per the split-TU guardrail. Final compare is 9/180 bytes different. The best source now matches the expected control flow, stack frame, bnezl null-load, pair.first stack reload, handle/second/pairSecond pointer roles, post-loop pair2 stores, SetSizeDirty call, and final flag stores. The visible remaining codegen difference is a local t2/t3 allocation swap in the loop body around handleSecondPtr/secondCopy: current emits addiu t2,a2,4; lw t3,0(a0); sw t3,0(t2), while expected wants addiu t3,a2,4; lw t2,0(a0); sw t2,0(t3), with t2 then reused for slti.

Tried the prior 172B pair-source baseline, volatile Pair, memory operands, fixed register declarations, local declaration reorders, explicit handleSecondPtr, volatile handleSecondPtr/first reload, and loop temporary register bindings. A 600s permuter run improved 11 bytes to 9 bytes but did not save; a follow-up 120s run with --save-best --save-improved reproduced and saved the 9-byte candidate. A manual swap of the t2/t3 register bindings did not change the allocation and worsened scheduling by moving the ra save earlier, so it was reverted. This looks like a near-final caller-save register allocation drift, but it is 9 bytes rather than the <=8-byte documented threshold, so it is left as ordinary failed/retryable rather than REG_ALLOC/unmatchable.
